class Appointment < ActiveRecord::Base
    ...
    if availability_on_day.nil?
	    ...
	    errors.add(:end_datetime, "Your appointment cannot end before it starts!") if end_datetime.hour < start_datetime.hour
	    errors.add(:end_datetime, "Your appointment runs too late.") if end_datetime.hour > availability_on_day.end_time
	  end
  end

  def duplicate_provider_booking?
  	errors.add(:end_datetime, "Your end time overlaps an existing appointment.") unless self.provider.appointments.where("? > start_datetime AND ? <= end_datetime", end_datetime, end_datetime).empty?
   ...
   def duplicate_client_booking?
  	return errors.add(:end_datetime, "You have another appointment at this time.") unless self.client.appointments.where("? > start_datetime AND ? <= end_datetime", end_datetime, end_datetime).empty?
